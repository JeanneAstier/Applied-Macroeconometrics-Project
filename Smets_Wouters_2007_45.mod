...
options_.drop = 0;
stoch_simul(irf = 100, graph);
*/
//solve_algo, order, etc. but here linear model

///////////////////////////////////////////////////////////////
// TEMPORARY DETERMINISTIC SIMULATION
///////////////////////////////////////////////////////////////
...
simul(periods = 1000);
M_.endo_names(27,:);
plot(oo_.endo_simul(27,1:40)); hold on;
plot(oo_.exo_simul(1:40,5));
...
plot(oo_.endo_simul(27,1:40)/0.1); hold on;
*/
...
simul(periods = 501);
M_.endo_names(27,:);
plot(oo_.endo_simul(27,1:100)); hold on;
*/
//Exact method
/*
initval; (<>0 wrt the new steady state)
endval; (=0 in the new steady state)
*/
...
//Full MC simulation
estimation(optim=('MaxIter',200),datafile=usmodel_data,mode_file=usmodel_shock_decomp_mode,mode_compute=0,first_obs=1,presample=4,lik_init=2,prefilter=0,mh_replic=20000,mh_nblocks=2,mh_jscale=0.20,mh_drop=0.2,posterior_graph,bayesian_irf,irf=150,irf_shocks=(em));

//Reload past MC with new draws
//estimation(optim=('MaxIter',200),datafile=usmodel_data,mode_file=usmodel_shock_decomp_mode,mode_compute=0,first_obs=1,presample=4,lik_init=2,prefilter=0,mh_replic=20000,mh_nblocks=2,mh_jscale=0.20,mh_drop=0.2,posterior_graph,bayesian_irf,irf=150,irf_shocks=(em),load_mh_file,load_results_after_load_mh);

//Reload past MC with no new draws
//estimation(optim=('MaxIter',200),datafile=usmodel_data,mode_file=usmodel_shock_decomp_mode,mode_compute=0,first_obs=1,presample=4,lik_init=2,prefilter=0,mh_replic=0,mh_nblocks=2,mh_jscale=0.20,mh_drop=0.2,nodisplay,load_mh_file,load_results_after_load_mh);
