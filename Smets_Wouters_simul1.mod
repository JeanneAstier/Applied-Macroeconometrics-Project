 ...
 *
 * It then simimlates data on 1000 periods after a productivity shock.
 */
...
//Full MC simulation 
estimation(optim=('MaxIter',200),datafile=usmodel_data,mode_file=usmodel_shock_decomp_mode,mode_compute=0,first_obs=1,presample=4,lik_init=2,prefilter=0,mh_replic=20000,mh_nblocks=2,mh_jscale=0.20,mh_drop=0.2,posterior_graph,bayesian_irf,irf=150,irf_shocks=(ea),nograph);
